...
# 8 "/Users/George/Documents/Programs/C/neovim/src/nvim/event/socket.h" 2
...
# 9 "/Users/George/Documents/Programs/C/neovim/src/nvim/event/socket.h" 2
...
void remote_ui_init(void)
{
  ...
  MsgpackRpcRequestHandler handler = {.fn = remote_ui_attach, .async = 0};
...
static void remote_ui_mode_change(UI *ui, int mode)
{
  Array args = {.size = 0, .capacity = 0, .items = ((void*)0)};
  if (mode == 0x10) {
    do { if ((args).size == (args).capacity) { (args).capacity = (args).capacity? (args).capacity<<1 : 8; (args).items = (Object*)xrealloc((args).items, sizeof(Object) * (args).capacity); } (args).items[(args).size++] = (((Object) { .type = kObjectTypeString, .data.string = cstr_to_string("insert") })); } while (0);
  } else if (mode == (0x40 + 0x10)) {
    do { if ((args).size == (args).capacity) { (args).capacity = (args).capacity? (args).capacity<<1 : 8; (args).items = (Object*)xrealloc((args).items, sizeof(Object) * (args).capacity); } (args).items[(args).size++] = (((Object) { .type = kObjectTypeString, .data.string = cstr_to_string("replace") })); } while (0);
  } else {
    (__builtin_expect(!(mode == 0x01), 0) ? __assert_rtn(__func__, "/Users/George/Documents/Programs/C/neovim/src/nvim/msgpack_rpc/remote_ui.c", 231, "mode == NORMAL") : (void)0);
    do { if ((args).size == (args).capacity) { (args).capacity = (args).capacity? (args).capacity<<1 : 8; (args).items = (Object*)xrealloc((args).items, sizeof(Object) * (args).capacity); } (args).items[(args).size++] = (((Object) { .type = kObjectTypeString, .data.string = cstr_to_string("normal") })); } while (0);
  }
