...
# 23 "/Users/George/Documents/Programs/C/neovim/src/nvim/msgpack_rpc/channel.c" 2
...
# 35 "/Users/George/Documents/Programs/C/neovim/src/nvim/msgpack_rpc/channel.c"
typedef enum {
  kChannelTypeSocket,
  kChannelTypeProc,
  kChannelTypeStdio
...
void channel_from_connection(SocketWatcher *watcher)
{
  ...
  rstream_init(&channel->data.stream, 0xffff);
                         ...
                         Error *err)
{
  ...
  do { int remaining = -1; uint64_t before = (remaining > 0) ? os_hrtime() : 0; while (!(frame.returned)) { do { if (channel->events && !queue_empty(channel->events)) { queue_process_events(channel->events); } else { loop_poll_events(&loop, remaining); } } while (0); if (remaining == 0) { break; } else if (remaining > 0) { uint64_t now = os_hrtime(); remaining -= (int) ((now - before) / 1000000); before = now; if (remaining <= 0) { break; } } } } while (0);
...
void channel_from_stdio(void)
{
  ...
  rstream_init_fd(&loop, &channel->data.std.in, 0, 0xffff,
      channel);
  rstream_start(&channel->data.std.in, parse_msgpack);

  wstream_init_fd(&loop, &channel->data.std.out, 1, 0, ((void*)0));
}
    ...
    void *data, _Bool eof)
{
  while (rbuffer_size(rbuf)) {
    ...
    buf[read] = '\000';
    do_log(3, __func__, 333, 1, "Channel %" "ll" "u" " stderr: %s", ((Channel *)data)->id, buf);
  }
}
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (handler.async) {
    ...
    queue_put_event(channel->events, event_create(1, on_request_event, 1, event_data));;
  }
}
...
static void close_channel(Channel *channel)
{
  ...
  switch (channel->type) {
    case kChannelTypeSocket:
      stream_close(&channel->data.stream, ((void*)0));
      break;
    case kChannelTypeProc:
      ...
      break;
    case kChannelTypeStdio:
      stream_close(&channel->data.std.in, ((void*)0));
      stream_close(&channel->data.std.out, ((void*)0));
      queue_put_event(loop.fast_events, event_create(1, exit_event, 1, channel));;
      return;
    default:
      abort();
  }
...
{
  do_log(3, __func__, 743, 1, "msgpack-rpc: %s", msg);
