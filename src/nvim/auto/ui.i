...
# 84 "/Users/George/Documents/Programs/C/neovim/src/nvim/ui.c"
...
{
    do { flush_cursor_update(); for (size_t i = 0; i < ui_count; i++) { UI *ui = uis[i]; if (ui->update_menu) ui->update_menu(ui); } } while (0);
}
...
{
  int new_col = col - 1;
  (__builtin_expect(!(new_col >= 0), 0) ? __assert_rtn(__func__, "/Users/George/Documents/Programs/C/neovim/src/nvim/ui.c", 461, "new_col >= 0") : (void)0);
  ui_cursor_goto(row, new_col);
}
...
{
  int new_col = col + 1;
  (__builtin_expect(!(new_col < width), 0) ? __assert_rtn(__func__, "/Users/George/Documents/Programs/C/neovim/src/nvim/ui.c", 468, "new_col < width") : (void)0);
  ui_cursor_goto(row, new_col);
}
...
static void ui_mode_change(void)
{
  ...
  if ((State & (0x40 + 0x10)) == (0x40 + 0x10))
    mode = (0x40 + 0x10);
  else if (State & 0x10)
    mode = 0x10;
  else
    mode = 0x01;
  do { flush_cursor_update(); for (size_t i = 0; i < ui_count; i++) { UI *ui = uis[i]; if (ui->mode_change) ui->mode_change(ui, mode); } } while (0);
